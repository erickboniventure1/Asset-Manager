...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.assets.update", [$asset->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label>{{ trans('cruds.asset.fields.donor') }}</label>
                <select class="form-control {{ $errors->has('donor') ? 'is-invalid' : '' }}" name="donor" id="donor">
                    <option value disabled {{ old('donor', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Models\Asset::DONOR_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('donor', $asset->donor) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('donor'))
                    <span class="text-danger">{{ $errors->first('donor') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.asset.fields.donor_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="model">{{ trans('cruds.asset.fields.model') }}</label>
                <input class="form-control {{ $errors->has('model') ? 'is-invalid' : '' }}" type="text" name="model" id="model" value="{{ old('model', $asset->model) }}" required>
                @if($errors->has('model'))
            ...
            <div class="form-group">
                <label class="required" for="supplier">{{ trans('cruds.asset.fields.supplier') }}</label>
                <input class="form-control {{ $errors->has('supplier') ? 'is-invalid' : '' }}" type="text" name="supplier" id="supplier" value="{{ old('supplier', $asset->supplier) }}" required>
